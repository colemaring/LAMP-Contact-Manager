...
function displaySearchedContacts()
{
    ...
    {
        contactList.innerHTML = "<h2 style=\"display:flex; flex: 1; justify-content: center;\">Contact does not exist.</h2>";
        return;
    }
...
function displayContactList()
{
    ...
    for (let i = numOfDisplayedContacts; i < contacts.length; i++)
    {
        ...
        + "<div><b>Name:</b><span> " + contacts[i].firstName + " " + contacts[i].lastName 
        + "</span></div> <div><b>Email:</b> <span>" + contacts[i].email + "</span></div>" 
        + "<div><b>Phone Number:</b> <span>" + contacts[i].phone + "</span></div>" 
        ...
        + "<ul class=\"dropdown-menu\" aria-labelledby=\"dropdownMenuButton1\"><li><button class=\"dropdown-item\" type=\"button\" id=\"update"+ i +"\" onclick=\"updateContact()\">Update</button></li>" 
        + "<li><button class=\"dropdown-item\" id=\"delete" + i + "\">Delete</button></li></ul></div></div></div>");
